...
.controller('HomeCtrl', function($scope,$state, $http, $ionicPopup, nfcService) {
	...
	console.log('testtttt'+ nfcService.tag);
...
.controller('TablesCtrl', function($scope, $state, $http, $stateParams){
  $scope.tables = JSON.parse($stateParams.listTables);
  ...
  /*$scope.getPizzaMkd = function(tableID, tableNum){
    ...
    var link = 'http://doukceri.alwaysdata.net/serveur/getPizzaMkd.php';
    $http.post(link).then(function (res){
      $scope.pizzasMkd = JSON.stringify(res.data);

      //Récupération des ingrédients
      var link = 'http://doukceri.alwaysdata.net/serveur/getIngredients.php';
      $http.post(link).then(function (res){
        $scope.ingredients = JSON.stringify(res.data);

        //Récupération des commandes
        var link = 'http://doukceri.alwaysdata.net/serveur/getCommande.php';
        $http.post(link, {tableID: tableID}).then(function (res){
          $scope.commandes = JSON.stringify(res.data);

          $state.go('pizzas', {tableID: tableID, tableNum: tableNum, pizzasMkd:$scope.pizzasMkd, ingredients:$scope.ingredients, commandes: $scope.commandes});
          });
      });
    });
  };*/
...
.controller('PizzasCtrl', function($scope, Pizzas, $stateParams, $state) {
  //Paramètres
  $scope.commandes = JSON.parse($stateParams.commandes);
  $scope.tableID = $stateParams.tableID;
  $scope.tableNum = $stateParams.tableNum;
  $scope.pizzasMkd = JSON.parse($stateParams.pizzasMkd);
  $scope.ingredients = JSON.parse($stateParams.ingredients);
  //Fonctions
  $scope.getPizza = function(pizzaId){
    $state.go('pizzaCustom', {tableID: $scope.tableID, tableNum: $scope.tableNum, pizzasMkd:$scope.pizzasMkd, ingredients:$scope.ingredients, pizzaId:pizzaId});
  };
